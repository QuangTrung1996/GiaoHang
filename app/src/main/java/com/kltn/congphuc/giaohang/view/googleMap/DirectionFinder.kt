...
import com.kltn.congphuc.giaohang.view.Main.fragmentMap
...
class DirectionFinder(listener: DetailBill, Latorigin: Double,Lgnorigin: Double, Latdestination: Double,Lgndestination:Double) {
    ...
    private var Latorigin: Double? = Latorigin
    private var Lgnorigin: Double? = Lgnorigin
    private var Latdestination: Double? = Latdestination
    private var Lgndestination: Double? = Lgndestination
...
//        val urlDestination = URLEncoder.encode(destination, "utf-8")
        return DIRECTION_URL_API + "origin=" + Latorigin.toString()+","+Lgnorigin.toString() + "&destination=" + Latdestination.toString()+","+Lgndestination.toString() + "&key=" + GOOGLE_API_KEY
    }
