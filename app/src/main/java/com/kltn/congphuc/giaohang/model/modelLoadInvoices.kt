...
class modelLoadInvoices(var responelloadVoinces: responelLoadVoinces) {
    fun loadDataInvoices(){
        val api: dataClient = APIUtils.getdata()
        val query:String = "{authenticatedShipper(email:\"Royce22@yahoo.com\",pass:\"kltnugao\"){_id,unPaidInvoices{ _id order_date price tasks{ receipt_date location{ address lat lng } } products{ product{ name } quantity } customer{ name phone _id } store{ location{ address lat lng } } } } }"
